...
from projects.blackjack.arts import logo
...
class BlackJack:
    ...
    def display_game_status(self):
        print(
            f"\tYour cards: [{', '.join(map(str, self.user_cards))}], current score: {sum(self.user_cards)}"
        )
        print(f"\tComputer's first card: {self.computer_cards[0]}")

    def display_final_hands(self):
        print(
            f"\tYour final hand: [{', '.join(map(str, self.user_cards))}], final score: {sum(self.user_cards)}"
        )
        print(
            f"\tComputer's final hand: [{', '.join(map(str, self.computer_cards))}], "
            f"final score: {sum(self.computer_cards)}"
        )

    def add_to_user_cards(self):
        self.user_cards.append(self.deal_card())
        self.user_score = self.calculate_score(self.user_cards)
        self.display_game_status()

    def add_to_dealer_cards(self):
        while self.computer_score < self.min_dealer_score:
            self.computer_cards.append(self.deal_card())
            self.computer_score = self.calculate_score(self.computer_cards)

    def continue_dealing_cards(self):
        return (
            self.user_score < self.blackjack
            and self.computer_score < self.blackjack
            and self.user_score != self.blackjack_score
            and self.computer_score != self.blackjack_score
        )

    def score_the_game(self):
        self.display_final_hands()
        if self.user_score == self.computer_score:
            print("Game is drawn.")
        elif self.user_score > self.blackjack:
    ...
    def start(self):
        while True:
            ...
            if game == "y":
                ...
                self.initiate_game()
                self.display_game_status()
                while self.continue_dealing_cards():
